...
export default {
  ...
  data() {
    ...
    created(){
        this.form.id=this.$store.state.orderInf.supermanageruserinf.UserID,
          this.form.name= this.$store.state.orderInf.supermanageruserinf.UserName,
          this.form.password= this.$store.state.orderInf.supermanageruserinf.Password,
          this.form.role=this.$store.state.orderInf.supermanageruserinf.Role
    },
    methods: {
        to(path){
      ...
      onSubmit() {
          var _this=this
          if(this.form.rolerole=='普通用户'){
            this.form.role=0
          }else if(this.form.role=='管理员'){
              this.form.role=1
          }else if(this.form.role=='超级管理员'){
              this.form.role=2
          }
        this.$http.post(this.HOST+'/useredit',{UserID:this.form.id,UserName:this.form.name,Password:this.form.password,Role:this.form.role},{
            headers: {
